BeforeAll {
    ...
    $validModulePath = Join-Path -Path $tempTestPath -ChildPath "ValidModule"
    ...
    $onlyPsm1ModulePath = Join-Path -Path $tempTestPath -ChildPath "OnlyPsm1Module"
    ...
    $onlyPsd1ModulePath = Join-Path -Path $tempTestPath -ChildPath "OnlyPsd1Module"
    ...
    $invalidPsd1ModulePath = Join-Path -Path $tempTestPath -ChildPath "InvalidPsd1Module"
...
Describe "deploy-ps-modules-to-azure.ps1" {
    ...
    It "should find only valid module folders with both .psd1 and .psm1 files containing valid ModuleVersion .psd1 key" {
        .\scripts\deploy-ps-modules-to-azure.ps1 @params
    ...
    It "should throw an error if no valid modules are found" {
        Remove-Item -Path "$tempTestPath\ValidModule" -Recurse -Force
